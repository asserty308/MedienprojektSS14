...
public class ScreenGUI : MonoBehaviour 
{
    ...
    void Start()
    {
		...
		m_currentLabelStyle.fontSize = 48;
	...
	void OnGUI()
    {
        GUI.Label(new Rect(20f, 10f, m_scoreLabel.Length * 3.5f, 20f), m_scoreLabel, m_scoreLabelStyle);

        if (m_showLabel)
        {
           GUI.Label(new Rect((Screen.width >> 1) - m_currentLabel.Length * 9.5f, Screen.height >> 2, m_currentLabel.Length * 7f, 20f), m_currentLabel, m_currentLabelStyle);
        }
    }
